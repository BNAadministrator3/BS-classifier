...
def stringchanging(ori):
    if ori == 'significantly present':
        return 'Present'
    elif ori == 'almost silent' :
        return 'Absent'
    elif ori == 'weak&&uncertainty':
        return 'Uncertain'
    else:
        print('Unreslved strings!')
        assert(0)
...
class buildTrainValtest():
    def __init__(self,datatable,labeltable):
        ...
        print('data size: {}'.format(len(datatable)))
        print()
    ...
    def cutintopieces(self,flag):
        ...
        for path,name in zip(testFpaths,TestFnames):
            ...
            for i in range(60):
                ...
                # print(i)
                tag = stringchanging(querylabel((i,i+1),self.labeltable[name]))
                assert(tag in ('Present','Absent','Uncertain'))
        ...
        for i in ('Present', 'Absent','Uncertain'):
    ...
    def getNonRepetitiveData(self,n_start, type='train', mark=False):
        ...
        maplist = {'Present': 2, 'Absent': 0,'Uncertain': 1}
        temp = data_label
        data_label = maplist[data_label]
        if mark is False:
            return data_input, np.array([data_label])
        else:
            return data_input, np.array([data_label]), stamp, temp
    ...
    def getData(self, n_start):  # Due to the class weight, samples in every batch does not need to be class-equal.
        ...
        data_input = []
        data_label = []
        for tag,offset in zip(('Present', 'Absent','Uncertain'),self.sampleoffset):
    ...
    def setCounting(self,dataset):
        ...
        assert(all([i in ('Present', 'Absent', 'Uncertain') for i in dataset.keys()]))
        return [len(dataset['Present']), len(dataset['Absent']), len(dataset['Uncertain'])]
